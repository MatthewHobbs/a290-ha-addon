...
A maintained Home Assistant add-on for the **Alpine A290** EV. It polls the
Renault/Kamereon API and publishes `sensor.alpine_a290_*` / `binary_sensor.*` / `button.*` /
`number.*` entities over **MQTT auto-discovery** — no shell scripts, no Python `venv`, no
`secrets.yaml`. Credentials are entered once on the **Configuration** tab.
