...
| `stale_hours` | How many hours the car may go without reporting before **Data Stale** turns on (default 6). This measures the age of the reading the car last sent, not how recently the add-on polled — a car can be silent for days while every poll succeeds. The same threshold also drives **Poll Failing**, which turns on when the add-on itself has not reached Renault for this long. |
